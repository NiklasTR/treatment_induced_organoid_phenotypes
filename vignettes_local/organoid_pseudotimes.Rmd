...
pca_raw <- readRDS(here("data/"))
# human data
# pca_feather <- feather::read_feather(here("data/embeddings/human_pca_feather"))
```
...
```{r}
clust <- readRDS(here("data/mclust/umap_mclust.Rds"))
clust_predict  <- readRDS(here("data/mclust/umap_mclust_predict.Rds")) # initially with 15 clusters
...
Next, I export a dataframe with a one-hot envoding of my data that I will use for classification subsequently.

```{r}
set.seed(123)

mco_pca_tidy_ml <- mco_pca_tidy %>% 
  mutate(negative = if_else(Drug == "DMSO", 1, 0),
         positive = if_else(Drug %in% c(lethal %>% as.character()), 1, 0), #"Bortezomib (PS-341)"
         # quick dummyfication 
         line_a = if_else(Line == "M001A03", 1, 0),
         line_b = if_else(Line == "M001B04", 1, 0),
         line_k = if_else(Line == "M001K02", 1, 0),
         line_w = if_else(Line == "M001W01", 1, 0),
         apc = if_else(Line %in% c("M001W01", "M001K02"), 0, 1),
         kras = if_else(Line %in% c("M001B04", "M001K02"), 1, 0)
         ) %>% 
  dplyr::select(id, contains("PC"), negative:kras) 

mco_pca_tidy_ml_split <- mco_pca_tidy_ml %>%
  filter(negative == 1 | positive == 1) %>%
  rsample::initial_split(strata = negative) 

mco_pca_tidy_ml %>%
  feather::write_feather(., here("data/train/mco_pca_tidy_complete"))

```

I run the ML in R using boosted random forest classifiers. 

```{r}
mco_pca_tidy_ml_split %>%
  training() %>%
  feather::write_feather(., here("data/train/mco_pca_tidy_train"))

mco_pca_tidy_ml_split %>% 
  testing() %>%
  feather::write_feather(., here("data/train/mco_pca_tidy_test"))
