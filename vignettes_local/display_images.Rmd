...
```{r}
ccp <- read_delim(here("layouts/Clin_Cancer_Panel_V170511.csv"), 
    ";", escape_double = FALSE, trim_ws = TRUE) %>% .[,-1] %>% 
  janitor::clean_names()

kistem <- read_delim(here("layouts/KiStem_edt_V170125.csv"), 
    ";", escape_double = FALSE, trim_ws = TRUE) %>% .[,-1] %>% 
  janitor::clean_names()
```
...
## new figure
name <- c("Docetaxel", "Paclitaxel", "DMSO")
set.seed(312)
