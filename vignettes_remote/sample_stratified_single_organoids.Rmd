...
saveRDS(hdf5_strat, here("hdf5_strat_100.Rds")) #hdf5_strat with only 25 objects per field
```

I load the dataset.

```{r, eval = TRUE}
hdf5_strat <- readRDS(here("hdf5_strat_100.Rds"))
```

To compare the stratified PCA embedding with a whole dataset based embedding, I load the first 25 PCs of the whole dataset.

```{r, eval = FALSE}
pca_path <- "/home/rindtorf/data/rauscher/promise/PROMISE/FeatureAnalysis/drug_effects/human/TransformedFeatures_human_25components.h5"
h5ls(pca_path)
...
keep_features <- hdf5_strat_miss %>% filter(n_miss == 0) %>% .$variable

hdf5_strat_filter <- hdf5_strat %>% 
  unnest(features) %>% 
  select(contains("V")) %>%
  #magrittr::set_colnames(h5read(poi, "feature_names_organoids")) %>% 
...
```{r}
inf_features <- hdf5_strat_filter %>% 
...
hdf5_strat_filter_inf <- hdf5_strat_filter %>% 
...
```{r}
umap_strat_100_meta <- hdf5_strat %>% 
  unnest(features) %>% 
  select(-contains("V")) %>% 
  semi_join(hdf5_strat, .) %>% 
...
I calculate the principal components of the dataset. 

```{r}
strat_pca <- hdf5_strat_filter_inf %>%
  as.matrix() %>%
  prcomp(scale = TRUE, center = TRUE)

var_pca <- strat_pca$x %>% as.data.frame() %>%
...
var_pca %>% 
  mutate(pc = factor(pc) %>% fct_inorder()) %>%
  ggplot(aes(pc, cum_var_exp)) + 
  geom_point() + 
  theme_classic() +
  geom_hline(yintercept =  0.9)

var_pca %>% 
  filter(cum_var_exp < 0.84) %>% tail(1)
```
...
```{r}
set.seed(123)
uwot_strat_100_euclid <- strat_pca$x %>% as.data.frame() %>% select(PC1:PC25) %>%
...
uwot_strat_100_cosine <- strat_pca$x %>% as.data.frame() %>% select(PC1:PC25) %>%
  # cbind(umap_dmso_100_meta) %>% 
  # filter(Line != "D010T01") %>%
  # mutate(Size = as.numeric(Size)) %>%
  # filter(Size > 1000) %>%
...
saveRDS(uwot_strat_100_euclid, here("uwot_strat_100_euclid.Rds"))
saveRDS(uwot_strat_100_cosine, here("uwot_strat_100_cosine.Rds"))


# For comparison I also create an embedding for the 25 pca-scaled datapoints with the same method.


set.seed(123)
uwot_bulk_100_euclid <- hdf5_pca %>% select(V1:V25) %>%
  # cbind(umap_dmso_100_meta) %>% 
  # filter(Line != "D010T01") %>%
  # mutate(Size = as.numeric(Size)) %>%
  # filter(Size > 1000) %>%
...
uwot_bulk_100_cosine <- hdf5_pca %>% select(V1:V25) %>%
  # cbind(umap_dmso_100_meta) %>% 
  # filter(Line != "D010T01") %>%
  # mutate(Size = as.numeric(Size)) %>%
  # filter(Size > 1000) %>%
...
# supervised approaches not useful 
# minus a does not make sense
saveRDS(uwot_bulk_100_euclid, here("uwot_bulk_100_euclid.Rds"))
saveRDS(uwot_bulk_100_cosine, here("uwot_bulk_100_cosine.Rds"))
```

I also test a compromise, which is a stratified embedding on global PCAs 

```{r}
set.seed(123)

hdf5_pca_sample <- hdf5_pca %>% 
group_by(Drug, Concentration, Plate) %>%
  sample_n(10, replace = TRUE) %>% 
  ungroup()

# uwot_bulk_100_sample_euclid <- hdf5_pca_sample %>% select(V1:V25) %>%
#   # cbind(umap_dmso_100_meta) %>% 
#   # filter(Line != "D010T01") %>%
#   # mutate(Size = as.numeric(Size)) %>%
#   # filter(Size > 1000) %>%
...
# 
# 
# uwot_bulk_100_sample_cosine <- hdf5_pca_sample %>%
#   # cbind(umap_dmso_100_meta) %>% 
#   # filter(Line != "D010T01") %>%
#   # mutate(Size = as.numeric(Size)) %>%
#   # filter(Size > 1000) %>%
...
#              ret_nn = TRUE)
# 
# saveRDS(uwot_bulk_100_sample_euclid, here("uwot_bulk_100_sample_euclid.Rds"))
# saveRDS(uwot_bulk_100_sample_cosine, here("uwot_bulk_100_sample_cosine.Rds"))

hdf5_pca_sample_meta <- hdf5_pca_sample %>% mutate(morphological_class = case_when(Line %in% c("D046T01",
                                                     "D054T01",
                                                     "D055T01") ~ "disorganized",
                                         Line %in% c("D018T01") ~ "organized",
                                         Line %in% c("D027T01",
...
uwot_bulk_100_sample_euclid <- readRDS(here("uwot_bulk_100_sample_euclid.Rds"))
uwot_bulk_100_sample_cosine <- readRDS(here("uwot_bulk_100_sample_cosine.Rds"))


tmp = reorient_umap(uwot_bulk_100_sample_euclid, hdf5_pca_sample_meta) 
tmp %>% umap_plot(file_name = "euclid_uwot_bulk_sample.pdf")
reorient_umap(uwot_bulk_100_sample_cosine, hdf5_pca_sample_meta) %>% umap_plot(file_name = "cosine_uwot_bulk_sample.pdf")

```


Based on the morphology of the embedding and the available literature on stability against batch effects, I focus on cosine distance as my distance of choice. 
...
```{r}
reorient_umap <- function(umap, umap_meta){
# I create a small function that centers the majority of the datapoints in the top-right corner of the embedding.
umap_oriented <- umap$embedding %>% 
  as.data.frame() %>%
  janitor::clean_names() %>%
  cbind(umap_meta) %>% as_tibble()

v1_center = median(umap_oriented$v1)
v2_center = median(umap_oriented$v2)

umap_oriented <- umap_oriented %>% 
  rowwise() %>%
  mutate(v1 = if_else(v1_center < 0, v1*(-1), v1),
         v2 = if_else(v2_center < 0, v2*(-1), v2))
return(umap_oriented)
}

umap_plot = function(umap_oriented, file_name){
  umap_oriented %>%
  # cbind(umap_dmso_100_meta %>% filter(Line != "D010T01") %>% 
  #         mutate(Size = as.numeric(Size)) %>%
  # filter(Size > 1000)) %>% 
  mutate(Drug = if_else(Drug == "DMSO", "DMSO", "Perturbation")) %>%
  ...
  theme(legend.position = "None") + 
    ggsave(here(file_name), width = 10, height = 10)
}
...
```{r}
#reading data 
uwot_strat_100_euclid <- readRDS(here("uwot_strat_100_euclid.Rds"))
uwot_strat_100_cosine <- readRDS(here("uwot_strat_100_cosine.Rds"))
uwot_bulk_100_euclid <- readRDS(here("uwot_bulk_100_euclid.Rds"))
uwot_bulk_100_cosine <- readRDS(here("uwot_bulk_100_cosine.Rds"))

uwot_bulk_100_sample_euclid <- readRDS(here("uwot_bulk_100_sample_euclid.Rds"))
uwot_bulk_100_sample_cosine <- readRDS(here("uwot_bulk_100_sample_cosine.Rds"))

#plotting
reorient_umap(uwot_strat_100_euclid, umap_strat_100_meta) %>% umap_plot(file_name = "euclid_uwot_strat.pdf")
reorient_umap(uwot_strat_100_cosine, umap_strat_100_meta) %>% umap_plot(file_name = "cosine_uwot_strat.pdf")
reorient_umap(uwot_bulk_100_cosine, hdf5_pca_meta) %>% umap_plot(file_name = "euclid_uwot_bulk.pdf")
reorient_umap(uwot_bulk_100_cosine, hdf5_pca_meta) %>% umap_plot(file_name = "cosine_uwot_bulk.pdf")

reorient_umap(uwot_bulk_100_sample_euclid, hdf5_pca_meta) %>% umap_plot(file_name = "euclid_uwot_bulk_sample.pdf")
reorient_umap(uwot_bulk_100_sample_cosine, hdf5_pca_meta) %>% umap_plot(file_name = "cosine_uwot_bulk_sample.pdf")
...
uwot_strat_100_oriented %>%
 ...
 #  filter(Size > 1000)) %>% 
  ...
  geom_density2d(color = "black") + 
  ggsave("uwot_strat_all.pdf", width = 6, height =4)
```
...
library(dbscan)
set.seed(123)
uwot_strat_100_cluster_10 <- uwot_strata_100$embedding %>% 
  as.data.frame() %>%
  janitor::clean_names() %>%
  hdbscan(., minPts = 10) #5 , 50, 10

saveRDS(uwot_strat_100_cluster_10, here("uwot_strat_100_cluster_10.Rds"))
```
