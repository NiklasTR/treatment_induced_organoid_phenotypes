...
When looking at this data, I see that the region in the upper-left corner is only present in a subset of organoid lines. When looking closer and comparing the experimental metadata with this observation, there is a real risk of these differences being batch effect. I load the experimental metadata.
...
stopifnot(pca_metadata %>% count(screen_ID, Line, Plate) %>% naniar::miss_summary() %>% .$miss_df_prop == 0)
```
...
pca_metadata_frac <- pca_metadata %>% sample_frac(0.01)
...
my_harmony_embeddings$Z_corr %>% t() %>% as_tibble()
#my_harmony_embeddings$Z_orig %>% t() %>% as_tibble()
```
...
reducedDims(ods_id)$PCA <- harmony_id$
```
