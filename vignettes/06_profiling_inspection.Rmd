...
```{r, eval = TRUE}
pca_effect_raw <- readRDS(here("vignettes/06_profiling_inspection/hdf5_pca.Rds"))
```

I test whether the number of plates in this dataset matches the raw feature dataset am I evaluated before. 

```{r}
plates = hdf5_pca_meta %>% dplyr::select(Plate) %>% distinct() %>% .$Plate

stopifnot(hdf5_path_df %>% mutate(pca = if_else(bc %in% plates, TRUE, FALSE)) %>% 
  filter(exist == TRUE) %>% 
  .$pca %>%
  min(as.numeric(.)) == 1)
...
pca_effect_raw %>% 
  ...
  ggsave(here("vignettes/06_profiling_inspection/overview_pca.png"))
...
pca_effect_raw %>% 
  ...
  theme_classic() + 
  ggsave(here("vignettes/06_profiling_inspection/count_hist.png"))
```
...
platetools::raw_grid(data = df$n, 
  ...
  ggsave(here("vignettes/06_profiling_inspection/count_map.png"))
```
...
platetools::raw_grid(data = df$Size, 
                     ...
                     plate = 384) + 
  scale_fill_viridis_c() + 
  ggsave(here("vignettes/06_profiling_inspection/size_map.png"))
```
...
segmentation_metrics %>% 
  ...
  ggsave(here("vignettes/06_profiling_inspection/outliers.png"))
```
...
```{r}
feature_path <- here::here("FeatureAnalysis/line_differences/human/DMSO/results/ReducedFeaturesPCA_noaa_human_DMSO.h5") 
...
```{r}
saveRDS(pca_dmso_raw, here("vignettes/06_profiling_inspection/hdf5_pca_absolute_dmso.Rds"))
```
...
saveRDS(features_raw, here("vignettes/06_profiling_inspection/hdf5_features.Rds"))
```
...
```{r, eval = FALSE}
saveRDS(pca_effect_raw_filtered, here("vignettes/06_profiling_inspection/hdf5_pca_filtered.Rds"))
```
