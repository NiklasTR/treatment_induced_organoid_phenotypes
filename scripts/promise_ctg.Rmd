...
##read ctg data files
ctg_data <- lapply(ctg_data, function(f) read_tsv(f, col_names=F, col_types = 'cci') %>%
...
```{r, results='hide', warning=F, message=F}
## select one plate
p <- ctg_data %>% filter(screen == '170620_LF_M3_D030T01P008L08')
...
p %>% extract(well, c('row', 'col'), regex='([A-P])(\\d{2})', remove=F) %>% 
  mutate(pcount = ifelse(drug != 'DMSO', -1000, pcount)) %>% 
...
p %>% extract(well, c('row', 'col'), regex='([A-P])(\\d{2})', remove=F) %>% 
  dplyr::select(row, col, pcount) %>% spread(col, pcount) %>% 
  dplyr::select(-row) %>% as.matrix() %>% pheatmap(cluster_rows=F, cluster_cols=F)
```

I would say that there is definitely a spacial bias where photon counts get higher towards the bottom of the plate.

```{r, results='hide', warning=F, message=F}
## scatter plot with robust loess fit.
ctg_data %>% extract(well, c('row', 'col'), regex='([A-P])(\\d{2})', remove=F) %>%
...
In order we correct for row-wise spacial bias we apply a loess-normalization to the data.

```{r, results='hide', warning=F, message=F}
## annotate rows and columns
ctg_data <- ctg_data %>% extract(well, c('row', 'col'), regex='([A-P])(\\d{2})', remove=F) %>%
  mutate(row=match(row, LETTERS)) 
...
ctg_loess %>% 
  ...
  ggplot(aes(screen, rv)) + geom_point() + facet_wrap(~rack.concentration) + theme_classic()
```

Without cross-plate normalization we can see that the drug is toxic, especially at higher concentrations, but we can definitely see some variation that is not necessarily expected. It might be explained by different growth rates of different lines. Let's see if a cross-plate normalization changes anything.
...
So it seems that a cross plate normalization doesn't actually matter, indicating that the growth rates might be affecting the variation in phenotype mostly. Let's see if this could be adjusted using a quantile normalization.

```{r, results='hide', warning=F, message=F}
## generate a matrix with a column per sample and a row per well
ctg_mat <- ctg_loess %>% dplyr::select(screen, well, pcount) %>% 
...
## normalize to dmso controls
ctg_qnorm <- ctg_qnorm %>% 
  left_join(ctg_qnorm %>% filter(drug == 'DMSO') %>% group_by(screen) %>% 
                summarise(ctrl = median(pc_norm, na.rm=T)) %>% ungroup()) %>% 
  group_by(screen) %>% mutate(rv = pc_norm/ctrl) %>% ungroup()
