...
```{r, results='hide', warning=F, message=F}
p %>% filter(screen == "170502_NR_M3_D007T01P006L08") %>% dplyr::select(row, col, pcount) %>% spread(col, pcount) %>% 
  dplyr::select(-row) %>% as.matrix() %>% 
  pheatmap(cluster_rows=F, cluster_cols=F)
```
...
ctg_loess <- ctg_data %>% select(row, col, pcount, screen) %>% split(.$screen) %>% lapply(function(s){
  ## loess fit. family is 'symmetric' to be robust to outliers
  fit <- loess(pcount ~ row + col, data=s, family='symmetric')
  ## apply normalization
  s %>% mutate(norm_fac = if_else(fit$n == 384, predict(fit), c(rep(NA, times = ))),
               pcount_norm = pcount - (norm_fac - median(norm_fac)))
}) %>% bind_rows()
```

```{r}
ctg_data %>% filter(screen == unique(ctg_data$screen)[1]) %>%
  dplyr::select(row, col, pcount) %>% spread(col, pcount) %>% 
  dplyr::select(-row) %>% as.matrix() %>% 
  pheatmap(cluster_rows=F, cluster_cols=F)
```
...
calc <- ctg_prolif %>% 
  select(timepoint_num, date, line, photons) %>%
  filter(timepoint_num %in% c(2,4)) %>% #there is no difference when starting the model at t=2/t=0
  group_by(line, date) %>%
  ...
  mutate(gr = log(`2`/`0`)/2,
         dt = log(2)/gr)
  # I also use a classic formula for growth rate calulcation. It should yield a similiar result
...
fit %>%
  ...
  ungroup(line) %>%
  as.data.frame() %>%
  mutate(line = factor(line, levels = line %>% as.vector() %>% unique())) %>% 
ggbarplot(x = "line", y = "m_mean",
...
#Doubling time
#x2 the time interval
#bargraph and error bars
