...
#CTG data analysis of HC1092 human organoid screen with clinically relevant compounds
...
Load the ctg datasets from the server cluster
```{r load-ctg, eval = FALSE}
...
drug_targets <- read_delim("~/promise/drug_targets.csv", 
    ";", escape_double = FALSE, trim_ws = TRUE) 
...
Show correlation coefficients betweeen all replicates
```{r}
...
##Generate treatment response curves and define compound activity
...
Generate panel of all screened compounds. Different response profiles are visible
```{r}
...
Draw a correlation plot of the screen replicates
```{r}
tmp <- ctg_ccp %>%
...
```{r calculate median centered log AUC for heatmap}
...
```{r access-dgidb-database, eval=FALSE}
...
write.csv(., "drug_targets_original.csv")
```
...
##Correction CTG data by proliferation rate
