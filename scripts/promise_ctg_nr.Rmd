...
save(tmp, file = paste0("promise/local_data/ctg_ccp_unfiltered.Rdata"))
#write results file 
...
```{r}
# <<<<<<< HEAD
 load(file = paste0("~/local_data/ctg_ccp_unfiltered.Rdata"))
# =======
#load(file = 'tmp.Rdata')
# >>>>>>> 9cf4c86d612a7582f9f37d38159ff48e58584e61
ctg_ccp <- tmp
```
...
###Show replicate-wise correlation coefficients (flagged)
```{r, include=FALSE}
...
###Response profiles with molecular subtype (flagged)
...
ctg_ccp %>%
  ...
  filter(drug %in% c(  "5-FU", "Irinotecan / SN-38", #YM-155, 5-FU, SN-38/Irinotecan, Venetoclax, Birinapant
                    "Venetoclax", "Birinapant", "YM155")) %>%
  mutate(line = as.factor(line),
         # drug = factor(drug)) %>%
...
##Propapoptotic compounds and CMS 
```{r}
dodge <- position_dodge(width=0.15)
ctg_ccp %>%
  #filter(drug %in% c( "Methotrexate", "Birinapant",  "Ponatinib",  "Nutlin3a", 
  ...
  filter(drug %in% c(  "5-FU", "Irinotecan / SN-38", #YM-155, 5-FU, SN-38/Irinotecan, Venetoclax, Birinapant
                     "Birinapant", "YM155")) %>%
  mutate(line = as.factor(line),
         drug = factor(drug, levels = c(  "5-FU", "Irinotecan / SN-38", #YM-155, 5-FU, SN-38/Irinotecan, Venetoclax, Birinapant
                     "Birinapant", "YM155"))) %>%
  left_join(., subtypes_organoids) %>%
  mutate(RF.predictedCMS = if_else(is.na(RF.predictedCMS), "no CMS", RF.predictedCMS),
         RF.nearestCMS = if_else(is.na(RF.nearestCMS), "no CMS", RF.nearestCMS)) %>%
  group_by(line, drug, concentration_factor) %>%
     dplyr::mutate(mean_photons = mean(treat.ctrl.median, na.rm = TRUE),
                     sd_photons = sd(treat.ctrl.median, na.rm = TRUE),
                     range_low_photons = range(treat.median)[1],
                     range_high_photons = range(treat.median)[2]) %>%
    ggplot(aes(y = mean_photons, x = concentration_factor)) + 
    geom_point(position = dodge,  stat = "identity", aes( colour = RF.nearestCMS), size = 2)  + 
    geom_path(aes(group = line, colour = RF.nearestCMS), alpha = 0.6, position = dodge, size = 2) + 
    #geom_errorbar(aes(ymin=mean_photons-sd_photons, ymax=mean_photons+sd_photons, group = line), width=0, position = dodge, size = 1, alpha = 0.5) +
    #geom_ribbon( aes(x=concentration_factor, y=mean_photons, ymin=mean_photons-sd_photons, ymax=mean_photons+sd_photons), alpha=0.2) +
    scale_x_log10(breaks = c(7,14)) + 
    ylab("Photon count normalized to control") + 
    facet_wrap(~drug) + 
    xlab("Concentration factor 5-fold") + 
    scale_color_manual(values=c("#377EB8", "#E41A1C", "#4DAF4A", "#9E9A99")) + 
    theme_minimal()
```
