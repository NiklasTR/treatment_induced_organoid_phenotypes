...
tsne_dat = readRDS("DrugEffects_AllDrugs_EffectLength_tSNE_human_25components.rds")
...
#   object = c("#A6CEE3", "#3F8EAA", "#79C360", "#B89B74", "#E52829",
#              "#FDB762", "#ED8F47", "#9471B4", "#DDD399", "#B15928"), 
#   nm = sort(unique(tsne_dat$Line)))
color_scale = setNames(
  object = c("#a6cee3", "#1f78b4", "#b2df8a", "#33a02c", "#fb9a99",
...
server <- function(input, output, session) {
  output$tsneplot <- renderPlotly({
    ...
    showControls = input$showOnlyControls
    
    unselected = tsne_dat
    
    gp = ggplot(mapping = aes(x = X, y = Y, text = sprintf(
      "Line: %s<br>Drug: %s<br>Target: %s<br>Mode of Action: %s", Line, Drug, Target, MOA))) + 
      geom_point(data = unselected, alpha = 0.5, color = "gray") + shiny_theme() + 
      scale_color_manual(values = color_scale) + 
      theme(legend.position = "none") + labs(color = "", shape = "") + 
      coord_cartesian(xlim = c(xmin, xmax), ylim = c(ymin, ymax))
    
    if(showControls == "all" | showControls == "only_ctrl") {
      dmso_control = unselected[unselected$Drug == "DMSO", ]
      bort_control = unselected[unselected$Drug == "Bortezomib", ]
      irino_control = unselected[unselected$Drug == "Irinotecan / SN-38", ]
      gp = gp + geom_point(data = dmso_control, mapping = aes(color = Line), size = 3, shape = 22) + 
        geom_point(data = bort_control, mapping = aes(color = Line), size = 3, shape = 24) + 
        geom_point(data = irino_control, mapping = aes(color = Line), size = 3, shape = 25)
    }
    
    if(showControls == "all" | showControls == "hide_ctrl") {
      ...
      gp = gp + geom_point(data = selected, mapping = aes(color = Line))
    }
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("selectedLines", "Lines",
                  ...
                  multiple = TRUE),
      radioButtons("showOnlyControls", "Toggle Controls", 
                   c("Show All" = "all", 
                     "Show Only Controls" = "only_ctrl", 
                     "Hide Controls" = "hide_ctrl"))
    ),
