...
```{r}
id = "D0"
dir = "PROMISE-10x-4t-c-16z"

temp <- list.files(path = paste0("/Volumes/collab/PROMISE/incoming/", dir), pattern = id, full.names = TRUE) %>%
  as_tibble() %>%
  mutate(time = file.info(.$value)$atime < Sys.time()-180,
         complete = !grepl("incomplete", .$value),
         csv = paste0(.$value, "_optimize_intensity.csv")) %>%
