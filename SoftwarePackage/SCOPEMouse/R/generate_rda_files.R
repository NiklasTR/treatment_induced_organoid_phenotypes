...
generate_rda_files_viability = function() {
  ...
  viability_directory = paste0(
    ...
    "FeatureAnalysis/organoid_viability/mouse")
  ...
  save(mortality, mortality_reduced, drug_annotations, roc_data, roc_aucs,
       acc_matrix, acc_matrix_reduced, file = "data/organoid_viability.RData")
...
generate_rda_files_drugeffects = function() {
  ...
  drug_effect_directory = paste0(
    "/Users/jansauer/Thesis/Projects/PROMISE/",
    "FeatureAnalysis/drug_effects/mouse")
  ...
  # Load replicate-wise profiles
  drug_effect_rep_metadata = list()
  drug_effect_rep_profiles = list()
  for(line in get_lines()) {
    md_fn = file.path(
      drug_effect_directory, line,
      sprintf("SVM_Accuracies_PCA_replicatewise_%s_%scomponents.csv",
              line, n_components))
    drug_effect_rep_metadata[[line]] = read.csv(md_fn, row.names = 1)
    drug_effect_rep_metadata[[line]]$Line = line
    drug_effect_rep_metadata[[line]]$Drug = rownames(drug_effect_rep_metadata[[line]])
    profiles_fn = file.path(
      drug_effect_directory, line,
      sprintf("SVM_Profiles_mean_PCA_replicatewise_%s_%scomponents.csv",
              line, n_components))
    drug_effect_rep_profiles[[line]] = read.csv(profiles_fn, row.names = 1)
  }
  drug_effect_rep_metadata = do.call(rbind, drug_effect_rep_metadata)
  drug_effect_rep_profiles = do.call(rbind, drug_effect_rep_profiles)

  # Reconstruct Replicate IDs
  drugsplit = strsplit(drug_effect_rep_metadata$Drug, "___")
  drug_effect_rep_metadata$Drug = sapply(drugsplit, "[[", 1)
  drug_effect_rep_metadata$Replicate = sapply(drugsplit, "[[", 2)

  save(drug_effect_profiles, drug_effect_metadata,
       drug_effect_rep_profiles, drug_effect_rep_metadata,
...
generate_rda_files_wellfeatures = function() {
  ...
  featuresdir = paste0(
    "/collab-ag-fischer/PROMISE/data-10x-4t-c-16z/",
    "features/")

  # Load well features
  plates = SCOPEMouse::get_plates()
  well_features = list()
  well_metadata = list()
  for(plate in plates) {
    h5f = h5file(
      name = file.path(featuresdir, plate, sprintf(
        "%s_averaged_features_organoids.h5", plate)),
      mode = "r")
    ...
    well_metadata[[plate]] = data.frame(t(
