...
#' @author Jan Sauer
#' 
#' @examples print(makeSortedIndexPage_old)
#' @export
makeSortedIndexPage_old <- function(platename, configdir) {
  warning("'makeSortedIndexPage_old(...)' is deprecated and should no longer be used")
  invisible(NULL)
  ...
  # Extract the negative and positive controls first
  treatments_NegCTRL = sort(unique(layout[layout$group == "NegCTRL", "substance"]))
  M_neg = lapply(treatments_NegCTRL, function(x) layout[layout$substance == x, "filename_small"])
  M_neg = do.call(what = rbind, args = M_neg)
  rownames(M_neg) = treatments_NegCTRL
  L_neg = lapply(treatments_NegCTRL, function(x) layout[layout$substance == x, "filename_large"])
  L_neg = do.call(what = rbind, args = L_neg)
  rownames(L_neg) = treatments_NegCTRL
  I_neg = lapply(treatments_NegCTRL, function(x) layout[layout$substance == x, "well"])
  I_neg = do.call(what = rbind, args = I_neg)
  rownames(I_neg) = treatments_NegCTRL
  
  treatments_PosCTRL = sort(unique(layout[layout$group == "PosCTRL", "substance"]))
  M_pos = lapply(treatments_PosCTRL, function(x) layout[layout$substance == x, "filename_small"])
  M_pos = do.call(what = rbind, args = M_pos)
  rownames(M_pos) = treatments_PosCTRL
  L_pos = lapply(treatments_PosCTRL, function(x) layout[layout$substance == x, "filename_large"])
  L_pos = do.call(what = rbind, args = L_pos)
  rownames(L_pos) = treatments_PosCTRL
  I_pos = lapply(treatments_PosCTRL, function(x) layout[layout$substance == x, "well"])
  I_pos = do.call(what = rbind, args = I_pos)
  rownames(I_pos) = treatments_PosCTRL
  
  # Extract the well names for each drug and sort them by concentration (-9, -3, 1, 3, 9)
  treatments = sort(unique(layout[layout$group == "drug", "substance"]))
  M = list()
  L = list()
  I = list()
  for(treatment in treatments) {
    sublayout = layout[layout$substance == treatment,]
    sublayout = sublayout[order(sublayout$concentration, decreasing = FALSE),]
    M[[treatment]] = sublayout$filename_small
    names(M[[treatment]]) = sublayout$concentration
    L[[treatment]] = sublayout$filename_large
    names(L[[treatment]]) = sublayout$concentration
    I[[treatment]] = sublayout$well
    names(I[[treatment]]) = sublayout$concentration
  }
