...
umap_sampled <- read_rds(paste0(PATH, "data/processed/PhenotypeSpectrum/filtered_moderate/umap_absolute_all_drugs_sampled.Rds"))

source(here("src/models/PhenotypeSpectrum/R/pseudotime.R"))
```
...
plot_grid(gg_bortezomib + coord_fixed(), gg_bortezomib_traj + coord_fixed(),
  ...
  ggsave(paste0(PATH, "reports/panel_bortezomib.pdf"), width = 8, height = 4)
...
```{r}
plot_grid(gg_irinotecan + coord_fixed(), gg_irinotecan_traj + coord_fixed(),
          labels = c('A', 'B'), label_size = 12, ncol = 2) +
  ggsave(paste0(PATH, "reports/panel_irinotecan.pdf"), width = 8, height = 4)
...
activity = umap_tidy %>% distinct(line, drug) %>%
  nest(-line) %>% 
  mutate(activity = purrr::map(line, ~ read_csv(paste0(PATH, "data/interim/FeatureAnalysis/drug_effects/human/",.x,"/SVM_Accuracies_PCA_",.x, "_25components.csv")) %>% 
                                        janitor::clean_names() %>% 
...
gg_mekgsk_density <- umap_sampled %>%
  filter(partition %in% c(1,2)) %>%
  dplyr::select(-line, -concentration) %>%
  ggplot(aes(v1, v2)) + 
  geom_point_rast(alpha = 1, size = 0.35, color = "#f1f1f1") + 
...
loi = "D019T01"
obj <- readRDS(paste0(PATH, "data/processed/PhenotypeSpectrum/drug_effects/lm_umap", loi, "_", i_dist_in, "_", j_nn_in, "_", k_res_in, ".Rds"))
...
```{r}
gg_target_density <- umap_sampled %>%
...
plot_grid(gg_target, gg_target_density,
  ...
  ggsave(paste0(PATH, "reports/enrichment_drugtarget.pdf"), width = 4, height = 4)
```
...
gg_traj_incl <- traj_df %>%
  ...
  ggplot(aes(v1, v2)) + 
  geom_point_rast(data = umap_sampled %>% dplyr::select(-line, -concentration), alpha = 1, size = 0.35, color = "#f1f1f1") + 
...
plot_grid(plot_grid(gg_traj_incl,gg_size_pc, labels = c('A', 'B'), label_size = 12, ncol = 2),
  ...
  ggsave(paste0(PATH, "reports/pseudotime_meki_", loi, "_", doi, ".pdf"), width = 8, height = 8)
```
...
gg_mek_traj_sel  + ggsave(paste0(PATH, "reports/figures/gg_mek_traj_sel.pdf"), width = 4, height = 4)
...
gg_big5_density + ggsave(paste0(PATH, "reports/figures/gg_big5_density.pdf"), width = 16, height = 8)
gg_mekgsk_density + ggsave(paste0(PATH, "reports/figures/gg_mekgsk_density.pdf"), width = 12, height = 12)
```
