...
# Also, I load similarly processed data, where only DMSO treated organoids were included in PCA
...
umap_tidy <- reducedDims(obj)$UMAP %>% cbind(colData(obj)) %>% as_tibble() %>% janitor::clean_names() %>% 
  left_join(., umap_tidy_h %>% dplyr::select(uuid, prob_live, prob_dead))
...
umap_tidy_dmso_h <- reducedDims(dmso_h)$UMAP %>% cbind(colData(dmso_h)) %>% as_tibble() %>% janitor::clean_names() %>% 
  left_join(., umap_tidy_h %>% filter(drug == "DMSO") %>% dplyr::select(uuid, prob_live, prob_dead))
...
umap_sampled_dmso_h <- umap_tidy_dmso_h %>%
  sample_frac(.1)
umap_sampled_dmso <- umap_tidy_dmso %>%
  sample_frac(.1)
```

# Line Differences

The data does not cluster by obvious phenotype. We hypothesize there is a bias by varying proportions of dead/missegmented organoids.
I evaluated 

```{r}

plot_pca_heatmap <- function(object, title, cut = 6){

  set.seed(1234234)
  
  pca_line_mat <- reducedDims(object)$PCA %>% cbind(colData(object)) %>% as_tibble() %>% janitor::clean_names() %>% 
    left_join(., umap_tidy_h %>% filter(drug == "DMSO") %>% dplyr::select(uuid, prob_live, prob_dead)) %>%
  #filter(prob_live >= 0.9) %>%
  filter(drug == "DMSO") %>% 
  filter(line != "D020T01") %>%
  group_by(line) %>% 
  summarize_at(vars(contains("pc")), funs(mean))
  
  pca_line_anno <- colData(object) %>% as_tibble() %>% janitor::clean_names()
...
plot_pca_heatmap(obj_h, "Harmony normalised, All data")
```

```{r}
plot_pca_heatmap(obj, "All data")
```

```{r}
plot_pca_heatmap(dmso_h, "Harmony normalised, DMSO data")
```

```{r}
plot_pca_heatmap(dmso, "DMSO data")
```
...
plot_grid(
  ...
  p3, p4,
  labels = "AUTO", ncol = 2
) +  ggsave(paste0(PATH, "notebooks/PhenotypeSpectrum/gg_size_panel.pdf"), 
            width = 8,
            height = 8)
...
plot_grid(
  ...
  p3, p4,
  labels = "AUTO", ncol = 2
) +  ggsave(paste0(PATH, "notebooks/PhenotypeSpectrum/gg_ldc_panel.pdf"),
            width = 8,
            height = 8)
...
plot_grid(
  ...
  p3, p4,
  labels = "AUTO", ncol = 2
) +  ggsave(paste0(PATH, "notebooks/PhenotypeSpectrum/gg_morph_panel.pdf"),
            width = 8,
            height = 8)
...
plot_grid(
  ...
  p3, p4,
  labels = "AUTO", ncol = 2
) +  ggsave(paste0(PATH, "notebooks/PhenotypeSpectrum/gg_line_panel.pdf"), 
            width = 8,
            height = 8)
...
plot_grid(
  ...
  p3, p4,
  labels = "AUTO", ncol = 2
) +  ggsave(paste0(PATH, "notebooks/PhenotypeSpectrum/gg_screen_panel.pdf"), 
            width = 8,
            height = 8)
...
intensity <- read_rds(here("data/interim/FeatureAnalysis/feature_intensity.rds")) %>%
  magrittr::set_colnames(c("actin", "permeability", "dapi"))

df <- cbind(umap_tidy, intensity)
...
df %>%
  filter(drug == "DMSO") %>%
  sample_n(10000) %>%
  ggplot(aes(v1, v2, color = log(permeability+10))) + 
...
```{r}
df_sum <- df %>% 
  group_by(line, replicate, drug, plate, screen_id) %>%
  summarise(mean_actin = mean(actin),
            sd_actin = sd(actin),
            mean_permeability = mean(permeability),
            sd_permeability = sd(permeability),
            mean_dapi = mean(dapi),
            sd_dapi = sd(dapi)
            )
...
df_sum %>% 
  filter(drug == 'DMSO') %>%
  ggplot(aes(plate, sd_permeability, color = screen_id)) + 
  geom_point() +
  theme_cowplot()
```
...
df_sum %>% 
  filter(drug == 'DMSO') %>%
  ggplot(aes(plate, sd_actin, color = screen_id)) + 
  geom_point() +
  theme_cowplot()
```
...
df_sum %>% 
  filter(drug == 'DMSO') %>%
  ggplot(aes(plate, sd_dapi, color = screen_id)) + 
  geom_point() +
  theme_cowplot()
```
...
p2 <- umap_morph_filter(umap_sampled, "Raw Data, All data")
...
plot_grid(
  p1, p2,
  p3, p4,
  labels = "AUTO", ncol = 2) +  ggsave(paste0(PATH, "notebooks/PhenotypeSpectrum/gg_morph_panel", loi, ".pdf"), 
            width = 8,
            height = 8)
...
lapply(umap_tidy_h$line %>% unique(), loi_morph)
```
