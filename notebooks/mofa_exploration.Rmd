...
model@cache$variance_explained$r2_per_factor$single_group %>% as.data.frame() %>% 
  ...
  mutate(overall_variance = expression + morphology_view + size_view) %>%
  mutate(overall_variance = overall_variance/3) %>%
...
weights %>% 
  rename(line = id) %>%
  left_join(organoid_size_fit %>% group_by(line) %>% summarise(size = mean(size))) %>%
...
weights %>% 
  rename(line = id) %>%
  left_join(organoid_size_fit %>% group_by(line) %>% summarise(size = mean(size))) %>%
  ggplot(aes(factor1, factor5, label = line, color = size)) + 
  geom_point() + 
  ggrepel::geom_text_repel() + 
  scale_color_viridis_c()
```
...
df = weights %>% 
  ...
  as.data.frame() %>%
  cbind(weights) %>% rename(line = id)

df %>% 
  ggplot(aes(V1, V2, color = factor1, label = line)) + 
  geom_point() + 
  ggrepel::geom_text_repel()
```
...
## integrating facots with drug data


```{r}
# defining a weight matrix with only single concentration treatment
weights_single_conc <- weights %>% 
  mutate(line = substr(id, 1, 7),
         img_group = substr(id, nchar(id), nchar(id)),
         expr_group = substr(id, nchar(id)-2, nchar(id)-2),
         drug = substr(id, 9, nchar(id)-4),
         conc_nan = substr(drug, nchar(drug)-2, nchar(drug))) %>% 
  filter(conc_nan == "NaN") %>% 
  mutate(drug = substr(drug, 1, nchar(drug)-4))
```


```{r}
umap_tidy_factor <- weights_single_conc %>% nest(-line, -drug) %>% mutate(order = purrr::map(data, ~ .x %>% mutate(img_group = as.numeric(img_group)) %>% arrange((img_group)) %>% cbind(replicate = c("1","2")))) %>% unnest(order) %>% dplyr::select(-data) %>%
  dplyr::select(-conc_nan) %>%
  left_join(umap_tidy)

set.seed(124)
umap_sampled_factor <- umap_tidy_factor %>% sample_frac(0.01)
```
