...
row_anno <-  df %>% left_join(loadings_drug_activity_tidy) %>% dplyr::select(drug, target) %>% as.data.frame() %>% column_to_rownames("drug") %>% distinct()
...
```{r}
drug_activity_test_result %>% filter(p.value <= 0.2) %>% 
  filter(term != "(Intercept)") %>% 
