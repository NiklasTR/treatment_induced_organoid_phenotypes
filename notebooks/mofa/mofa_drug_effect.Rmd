...
```{r}
# input
model <- load_model(here("models/mofa/model_oldcode_newdocker.hdf5"))
...
drug_projection_test_result_drug <- rbind(
  lm(V1 ~ drug, data = z_projected_anno_test) %>% summary() %>% broom::tidy() %>% mutate(factor = "factor1"),
  lm(V2 ~ drug, data = z_projected_anno_test) %>% summary() %>% broom::tidy() %>% mutate(factor = "factor2"),
  lm(V3 ~ drug, data = z_projected_anno_test) %>% summary() %>% broom::tidy() %>% mutate(factor = "factor3")) 
