---
title: "Data Normalization and QC (Human)"
author: "Jan Sauer"
date: "12.10.2017"
output: 
  html_document:
...
Selecting features based on quality statistics, such as the z-factor, may introduce biases. For example, one might ignore features that differentiate specific treatments but not the controls from each other. Instead, I directly perform a stepwise feature selection over the entire screen, as per Fischer et al. 2015 ("A Map of Directional Genetic Interactions in a Metazoan Cell") and Laufer et al. 2013 ("Mapping genetic interactions in human cancer cells"). This entails the following steps:
...
To reduce the runtime of the feature selection, I remove strongly correlating features ($\rho$ >= +/-0.90).
```{r}
...
features_reduced <- features[,!apply(features_cor,2,function(x) any(abs(x) > 0.90))]
```

```{r}
# Set up initial features
initial_set = c("organoids_num.of.objects", "Total.Biomass")
  ...
  for(feat in remaining_features) {
    model1 = lm(
      features_reduced[features_metadata$REPLICATE == 1, feat] ~
        as.matrix(features_reduced[features_metadata$REPLICATE == 1, selected_features]))
    model2 = lm(
      features_reduced[features_metadata$REPLICATE == 2, feat] ~ 
        as.matrix(features_reduced[features_metadata$REPLICATE == 2, selected_features]))
