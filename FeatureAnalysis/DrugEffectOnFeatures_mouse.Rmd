...
basedir = "/Users/jansauer/Thesis/Projects/PROMISE/FeatureAnalysis"
# basedir = file.path(basedir, "results")
# if(!dir.exists(basedir)) dir.create(basedir, showWarnings = FALSE, recursive = FALSE)

# Change this to match the full path to the configdir on your system
configdir = "/collab-ag-fischer/PROMISE/data-10x-4t-c-16z/configdir"
# configdir = "/Users/jansauer/tmp/watchdog_local/configdir"
...
A function to load the median features from memory. The median feature files should be in the directory '$basedir/features' and have the format 'M001W01_features_averaged_organoids.h5'

```{r load_features}
get_median_features = function(cell_line, basedir) {
  possible_cell_lines = c("W", "A", "K", "B")
  if(!cell_line %in% possible_cell_lines) {
    warning(sprintf("'cell_line' must be in (%s)", paste0(possible_cell_lines, collapse = ",")))
    return(NULL)
  }
  fname = list.files(
    file.path(basedir, "features"), 
    pattern = sprintf("^M001%s0[1-9]{1}_features_averaged_organoids.h5$", cell_line))
  if(length(fname) != 1) {
    warning(sprintf("Multiple possible feature files found for '%s': %s", cell_line, as.character(fname)))
    return(NULL)
  }
  hdf5_cl = h5read(file.path(basedir, "features", fname), "cell_line")
  hdf5_ft = h5read(file.path(basedir, "features", fname), "feature_type")
  
  if(substr(hdf5_cl, 1, 5) != sprintf("M001%s", cell_line)) {
    warning(sprintf("Critical Error with the cell_line in file '%s'", fname))
    return(NULL)
  }
  
  if(hdf5_ft != "organoids") {
    warning(sprintf("Critical Error with the feature_type in file '%s'", fname))
    return(NULL)
  }
  
  noise = as.data.frame(h5read(file.path(basedir, "features", fname), "features_noise"))
  signal = as.data.frame(h5read(file.path(basedir, "features", fname), "features_signal"))
  feature_names = h5read(file.path(basedir, "features", fname), "feature_names")
  well_names = h5read(file.path(basedir, "features", fname), "well_names")
  
  colnames(noise) = colnames(signal) = feature_names
  rownames(noise) = rownames(signal) = well_names
  
  # Remove reimaged plates (any plate that has a 9XX variant should be chosen over the 0XX variant)
  well_plates = sapply(strsplit(well_names, "_"), "[[", 1)
  all_plates = unique(well_plates)
  ...
  indices = well_plates %in% filtered_plates
  noise = noise[indices,]
  signal = signal[indices,]
  well_names = well_names[indices]
  well_plates = well_plates[indices]
  if(!identical(rownames(noise), rownames(signal))) {
    warning(sprintf(paste0(
      "Feature names for 'noise' and 'signal' data sets", 
      "aren't identical but should be! ('%s')"), fname))
    return(NULL)
  }
  colnames(noise) = paste0("noise_", colnames(noise))
  colnames(signal) = paste0("signal_", colnames(signal))
  combined_feats = cbind(signal, noise)
  ...
  replicate_dict = setNames(
    object = (filtered_plates %in% rep2_plates) + 1,
    nm = filtered_plates)
  combined_feats$REPLICATE = replicate_dict[well_plates]
  ...
  combined_feats$DRUG = as.character(layouts[substr(well_names, 12, 18),"Product.Name"])
  return(combined_feats)
}
```

```{r}
cell_lines = c("W", "A", "K", "B")
features = setNames(object = vector(mode="list", length=length(cell_lines)), nm = cell_lines)
for(cell_line in cell_lines) {
  features[[cell_line]] = get_median_features(cell_line, basedir)
  features[[cell_line]]$CELL.LINE = cell_line
}
all_well_names = do.call(c, lapply(features, rownames))
features = as.data.frame(rbindlist(l = features, use.names = TRUE, fill = FALSE))
rownames(features) = all_well_names
```

### Feature selection

The first step of feature selection involves removing any obviously useless features, i.e. features that are constant across all wells within either replicate.

```{r}
# Strip the matrix down to the feature columns
non_feature_cols = which(colnames(features) %in% c("REPLICATE", "DRUG", "CELL.LINE"))
features_stripped = features[,-non_feature_cols]

# Remove empty features
feat_range = aggregate(
  features_stripped, by = list(features$REPLICATE), 
  function(x) max(x) - min(x))
feat_range$Group.1 = NULL
valid_features = apply(feat_range, 2, function(x) sum(x > 0) == 2)
features_stripped = features_stripped[,valid_features]
```

Compute the correlations between replicates of features and keep only features that correlate with at least 0.6 between replicates
```{r}
rep1 = features_stripped[features$REPLICATE == 1,]
rep2 = features_stripped[features$REPLICATE == 2,]
...
if(!identical(substr(rownames(rep1), 12, 18), substr(rownames(rep2), 12, 18))) {
  warning("Wells are NOT in the same order between wells!")
}
...
ggplot(data = ggplotdf) + geom_line(aes(x = Features, y = Correlation)) + 
  geom_hline(aes(yintercept = 0.6, color = "red")) + theme(legend.position = "none")

features_stripped = features_stripped[,correlations >= 0.6]
```
