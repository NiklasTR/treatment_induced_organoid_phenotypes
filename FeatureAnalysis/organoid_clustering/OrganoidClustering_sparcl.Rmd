...
Keep only focused organoids
```{r}
features = features[object_type != "BLURRY", ]
object_type = object_type[object_type != "BLURRY"]
```
...
# Gap Statistic
I take a look at the gap statistic (Tibshirani et al, 2000; Grün et al, 2015) to determine the optimal number of clusters.
```{r}
pam1 = function(x, k) list(cluster = pam(x, k, cluster.only = TRUE))
gapstat = clusGap(x = features, FUNcluster = pam1, K.max = 20, B = 50, verbose = FALSE)
...
I use a sparse hierarchical clustering approach
```{r}
hclust = HierarchicalSparseCluster(features, method = "centroid", niter = 50, silent = TRUE)
colors = c("red", "blue", "green", "black")
plot.phylo(
  as.phylo(hclust$hc), cex = 0.6, label.offset = 0.5, type = "phylogram", 
  show.tip.label = FALSE, edge.color = colors[as.factor(object_type)], 
  tip.color = colors[as.factor(object_type)])
legend(x="topleft",legend = levels(as.factor(tolower(object_type))), fill = colors,xpd = T, cex = 1)
```

# K-Means Clustering
```{r}
clus = KMeansSparseCluster(features_norm, K = 6, silent = TRUE)
```
...
I attempt to recreate this split using only those objects. While the optimal number of clusters is clearly k = 1, a significant local maximum can also be seen at k = 7
```{r}
gapstat = clusGap(
  x = features[object_type == "Organoid", ], FUNcluster = pam1, 
