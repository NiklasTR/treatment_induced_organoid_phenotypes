...
for(concentration in sort(unique(drug_metadata$CONCENTRATION))) {
  ...
  source("svmrfe.R")
  svmrfe = svm_rfe(X = x_train, y = y_train, X_test = x_test, y_test = y_test, elim_frac = 0.1)
  full_model = ksvm(x = x_train, y = y_train, scaled = TRUE, kernel = "vanilladot")
  full_acc = mean(predict(full_model, x_test) == y_test)
  
  # Remove features whose removal doesn't cause the accuracy to drop below 90% of the full model accuracy
  # "Remove" means that the features are set to 0
  max_rank_to_keep = tryCatch(
    max(svmrfe$Ranking[svmrfe$Accuracy <= full_acc*0.9] + 1), 
    warning = function(w) 1)
  if(length(max_rank_to_keep) == 0) max_rank_to_keep = 1
  features_to_keep = colnames(x)[svmrfe$Ranking <= max_rank_to_keep]
  reduced_model = ksvm(x = x_train[, features_to_keep], y = y_train, scaled = TRUE, kernel = "vanilladot")
  reduced_acc = mean(predict(reduced_model, x_test[, features_to_keep]) == y_test)
  
  conc_acc[[as.character(concentration)]] = reduced_acc
  
  # # Calculate normal vector to hyperplane as profile
  norm_vec_vals = (reduced_model@coef[[1]] %*% reduced_model@xmatrix[[1]])[1,]
  norm_vec = setNames(
   object = rep(0, ncol(conc_features)), 
   nm = colnames(conc_features))
  norm_vec[names(norm_vec_vals)] = norm_vec_vals
  conc_vec[[as.character(concentration)]] = norm_vec
  
  # Calculate the distance and vector between the groups
  # drug_centroid = colMeans(x[y == "DRUG", features_to_keep])
  # dmso_centroid = colMeans(x[y == "DMSO", features_to_keep])
