...
data_dir = "/Users/jansauer/Thesis/Projects/PROMISE/FeatureAnalysis/python_package/dead_organoid_classifier/full_classifier"
...
mortality = list()
for(plate in all_plates) {
  # dat = read.csv(file.path(data_dir, plate, sprintf("%s_organoid_classification_full_classifier.csv", plate)))
  dat = read.csv(file.path(data_dir, sprintf("%s_organoid_classification_full_classifier.csv", plate)))
...
for(drug in all_drugs) {
  ...
  ggsave(filename = sprintf("dose_response/full_classifier/DoseResponse_%s.pdf", drug_name), width = 10, height = 7)
}
```
