...
hdf5_dataset = paste0("features_zscore_rep_", feature_type)
```
...
* Separate objects into "organoids" (area >= 1500 pixels) and "shrapnel" (area <= 1500 pixels).
...
Keep features that correlate between replicates within each cell line and across the entire screen with $\rho >= 0.4$. I also ensure that the number of organoids and the total biomass are guaranteed to be in the dataset (regardless of their correlation).
```{r}
cor_thresh = 0.4
...
if(!file.exists("QC_human__stability_selection__features.rds")){
